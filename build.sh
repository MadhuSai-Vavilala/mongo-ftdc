...
if [ "$1" == "docker" ]; then
	VER=$(cat version)
	TAG="MadhuSai-Vavilala/mongo-ftdc"
    ...
    docker build --no-cache -f grafana/Dockerfile -t MadhuSai-Vavilala/grafana-ftdc:${VER} .
    docker tag MadhuSai-Vavilala/grafana-ftdc:${VER} MadhuSai-Vavilala/grafana-ftdc
else
